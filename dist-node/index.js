...
async function scanImports(cwd, {
  ...
  const loadedFiles = await Promise.all(includeFiles.map(async filePath => {
    ...
    if (ext === '.js' || ext === '.mjs' || ext === '.ts' || ext === '.jsx' || ext === '.tsx') {
      return [filePath, await fs__default.promises.readFile(filePath, 'utf-8')];
    }
...
function checkIsEsModule(fileLoc) {
  const code = fs__default.readFileSync(fileLoc, 'utf8');
...
function filterPackageIsEsModule(externalPackages) {
  return externalPackages.map(spec => [spec, resolveWebDependency(spec)]).filter(([spec, {
    type,
    loc
  }]) => type === 'JS').filter(([spec, {
    type,
    loc
  }]) => checkIsEsModule(loc)).map(([spec]) => spec);
}
...
async function install(installTargets, {
  ...
  const autoDetectNamedExports = [...CJS_PACKAGES_TO_AUTO_DETECT, ...config.installOptions.namedExports]; // We run some special processing steps when installing a single package.
...
async function command({
  ...
  if (finalResult) {
    spinner.succeed(chalk.bold(`snowpack`) + ` install complete${spinnerHasError ? ' with errors.' : '.'}` + chalk.dim(` [${((Date.now() - startTime) / 1000).toFixed(2)}s]`));
...
async function command$1(commandOptions) {
  ...
  const configSrcFileExtensionMapping = deepmerge.all([{}, srcFileExtensionMapping, config.extensionMap || {}]); // Start with a fresh install of your dependencies, for production
  ...
  commandOptions.config.installOptions.treeshake = commandOptions.config.installOptions.treeshake !== undefined ? commandOptions.config.installOptions.treeshake : true;
  const dependencyImportMapLoc = path.join(config.installOptions.dest, 'import-map.json'); // Start with a fresh install of your dependencies, always.
...
function getUrlFromFile(mountedDirectories, fileLoc, extensionMapping) {
  for (const [dirDisk, dirUrl] of mountedDirectories) {
    if (fileLoc.startsWith(dirDisk + path.sep)) {
      ...
      return fileLoc.replace(dirDisk, resolvedDirUrl).replace(/[/\\]+/g, '/').replace(new RegExp(`${fileExt}$`), extensionMapping[fileExt] || fileExt);
    }
  }
...
async function command$2(commandOptions) {
  ...
  const configSrcFileExtensionMapping = deepmerge.all([{}, srcFileExtensionMapping, config.extensionMap || {}]);
        ...
        if (isRoute) {
          ...
          for (const workerConfig of config.scripts) {
            ...
            for (const extMatcher of match) {
              if (extMatcher === requestedFileExt.substr(1) || configSrcFileExtensionMapping[extMatcher] === requestedFileExt.substr(1)) {
  ...
  const createServer = credentials ? requestHandler => http2.createSecureServer(_objectSpread2(_objectSpread2({}, credentials), {}, {
    allowHTTP1: true
  }), requestHandler) : requestHandler => http.createServer(requestHandler);
...
function normalizeConfig(config) {
  ...
  config.plugins = config.plugins.map(plugin => {
    ...
    if (configPlugin.extensionMap) {
      config.extensionMap = deepmerge.all([{}, config.extensionMap || {}, configPlugin.extensionMap]);
    }
...
function loadAndValidateConfig(flags, pkgManifest) {
  ...
  const mergedConfig = deepmerge.all([pkgManifest.homepage ? {
    buildOptions: {
